...
source=("${_pkgapp}-v8.16.30.AppImage::https://github.com/RPGLogs/Uploaders-warcraftlogs/releases/download/v8.16.30/warcraftlogs-v8.16.30.AppImage"
        'start')
license=('custom' 'MIT' 'custom:chromium-licenses')
options=(!strip)
# Skip checksum check for the WarcraftLogs binary, to avoid breakage on updates
sha512sums=('d47a0310d9c39c40c7c334c90efc50b2031f21cd7b300358e76ffb3841c5536edc754b95adab4b14037fb2a7ede32f71bb55d982aa8bee1c4d1f07b57eb5d056'
...
pkgver() {
    cd ${srcdir}
    chmod +x ${srcdir}/${_pkgapp}-v8.16.30.AppImage
    ${srcdir}/${_pkgapp}-v8.16.30.AppImage --appimage-extract >/dev/null
...
package() {
    cd ${srcdir}
    chmod +x ${srcdir}/${_pkgapp}-v8.16.30.AppImage
    ./${_pkgapp}-v8.16.30.AppImage --appimage-extract >/dev/null
    sed -i 's/Exec=.*/Exec=\/usr\/bin\/'${_pkgapp}' %U/' squashfs-root/warcraftlogs.desktop

    install -Dm755 ${_pkgapp}-v8.16.30.AppImage "${pkgdir}/opt/${_pkgapp}/${_pkgapp}.AppImage"
