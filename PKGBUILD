...
source=("${_pkgapp}-v8.15.14.AppImage::https://github.com/RPGLogs/Uploaders-warcraftlogs/releases/download/v8.15.14/warcraftlogs-v8.15.14.AppImage"
        'start')
license=('custom' 'MIT' 'custom:chromium-licenses')
options=(!strip)
# Skip checksum check for the WarcraftLogs binary, to avoid breakage on updates
sha512sums=('4d68944ea956528c279f98e7586946f660ca93dd3bdf4d9bcbb0374ecf79854b3b663b5220b03256b98a1461d8189d3fa60031b80e706b44f3732c45a5cb4389'
...
pkgver() {
    cd ${srcdir}
    chmod +x ${srcdir}/${_pkgapp}-v8.15.14.AppImage
    ${srcdir}/${_pkgapp}-v8.15.14.AppImage --appimage-extract >/dev/null
...
package() {
    cd ${srcdir}
    chmod +x ${srcdir}/${_pkgapp}-v8.15.14.AppImage
    ./${_pkgapp}-v8.15.14.AppImage --appimage-extract >/dev/null
    sed -i 's/Exec=.*/Exec=\/usr\/bin\/'${_pkgapp}' %U/' squashfs-root/warcraftlogs.desktop

    install -Dm755 ${_pkgapp}-v8.15.14.AppImage "${pkgdir}/opt/${_pkgapp}/${_pkgapp}.AppImage"
